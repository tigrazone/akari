...
private:
	struct HDRPixel {
		...
		HDRPixel(const Color &color) {
			double d = std::max(color.x_, std::max(color.y_, color.z_));
			if (d <= 1e-32) {
				r_ = g_ = b_ = e_ = 0;
				return;
			}
			int e;
			const double m = frexp(d, &e); // d = m * 2^e
			d = m * 256.0 / d;

			r_ = (unsigned char)(color.x_ * d);
			g_ = (unsigned char)(color.y_ * d);
			b_ = (unsigned char)(color.z_ * d);
			e_ = (unsigned char)(e + 128);
		}
	};
	...
	void load_unsafe(const std::string &filename) {
		...
		for (int y = 0; y < height_; ++y) {
			for (int x = 0; x < width_; ++x) {
				...
				image_[(height_ - 1 - y) * width_ + x].x_ = tmp_data[(y * width_ + x) * 4 + 0] * pow(2, e - 128.0f) / 256.0f;
				image_[(height_ - 1 - y) * width_ + x].y_ = tmp_data[(y * width_ + x) * 4 + 1] * pow(2, e - 128.0f) / 256.0f;
